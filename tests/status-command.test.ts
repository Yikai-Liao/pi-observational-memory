...
function memoryEntries(): any[] {
	const a = { id: "o1", content: "Investigated a detailed issue and recorded the durable technical findings for later work.", sourceEntryIds: ["raw-a"] };
	const b = { id: "o2", content: "Implemented the detailed fix and verified the durable behavior with focused tests.", sourceEntryIds: ["raw-b"] };
...
describe("/om:status", () => {
	it("reports tree, cadence, rendering, and validation", async () => {
		...
		const notify = vi.fn();
		await handler("", { cwd: "/project", model: { contextWindow: 100_000 }, sessionManager: { getEntries: memoryEntries, getBranch: memoryEntries }, ui: { notify } });
	...
	it("reports passive mode", async () => {
		...
		const notify = vi.fn();
		await handler("", { cwd: "/project", sessionManager: { getEntries: memoryEntries, getBranch: memoryEntries }, ui: { notify } });
