...
describe("V4 observation coverage", () => {
	it("ignores malformed events and coverage markers outside the branch", () => {
		...
		const entries: Entry[] = [
			...
			raw("raw-b"),
			{ type: "custom", id: "malformed", customType: OM_OBSERVATIONS_RECORDED, data: { version: 1, nodeRecords: [], segmentCheck: "not_requested" } },
			{ type: "custom", id: "missing-coverage", customType: OM_OBSERVATIONS_RECORDED, data: { version: 1, nodeRecords: [observation], coversUpToId: "not-on-branch", segmentCheck: "not_requested" } },
		];
